...
function HistoryCard({ entry, onView }: { entry: MoveHistoryEntry; onView: () => void }) {
  return (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500">{entry.moveType} {entry.moveSize}</span>
          </div>
          <p className="text-sm font-medium text-navy-900 truncate">{entry.fromAddress}</p>
          <div className="flex items-center gap-1 my-0.5">
            <div className="h-px flex-1 bg-slate-200 max-w-8" />
            <ArrowRight className="w-3 h-3 text-slate-400" />
          </div>
          <p className="text-sm font-medium text-navy-900 truncate">{entry.toAddress}</p>
        ...
        <div className="text-right flex-shrink-0">
          ...
          <button onClick={onView} className="mt-2 flex items-center gap-1 text-xs font-medium text-navy-600 hover:text-navy-800 cursor-pointer transition-colors">
            View Summary <ChevronRight className="w-3 h-3" />
          </button>
        </div>
      </div>
      <p className="text-xs text-slate-400 mt-2.5 pt-2.5 border-t border-slate-100">Available online for 90 days</p>
    </div>
  );
}
...
export default function SummaryPage() {
  ...
  function handleEmailSend() {
    ...
    setTimeout(() => { setShowEmailField(false); setEmailInput(""); setEmailSent(false); }, 2000);
  }
    ...
    <main className="min-h-screen bg-slate-50 pb-24">
      <header className="bg-navy-900 px-5 pt-4 pb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <a href="/status" className="text-navy-300 text-xs hover:text-white transition-colors">← Back to status</a>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-navy-300 text-xs mt-0.5">Thank you for choosing One Stop Moving &amp; Storage</p>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 py-5 max-w-lg mx-auto space-y-4">
          ...
          <div className="px-4 py-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-start gap-2">
                ...
                <div><p className="text-xs text-slate-500">Date</p><p className="text-sm font-medium text-slate-800">Mar 12, 2026 · Morning</p></div>
              </div>
              <div className="flex items-start gap-2">
                <Users className="w-4 h-4 text-slate-400 mt-0.5" />
                <div><p className="text-xs text-slate-500">Lead mover</p><p className="text-sm font-medium text-slate-800">Marcus T.</p></div>
              </div>
              <div className="flex items-start gap-2">
                <Truck className="w-4 h-4 text-slate-400 mt-0.5" />
                <div><p className="text-xs text-slate-500">Move type</p><p className="text-sm font-medium text-slate-800">{move.moveType} · {move.moveSize}</p></div>
              </div>
              <div className="flex items-start gap-2">
                <Clock className="w-4 h-4 text-slate-400 mt-0.5" />
                <div><p className="text-xs text-slate-500">Crew &amp; truck</p><p className="text-sm font-medium text-slate-800">Crew of 3 · 26ft truck</p></div>
              </div>
            </div>
            <div className="h-px bg-slate-100" />
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 flex-1">
                ...
                <div><p className="text-xs text-slate-500">Est. weight</p><p className="text-sm font-medium text-slate-800">~1,200 kg</p></div>
              </div>
              <div className="flex-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          ...
          <ul className="space-y-2">
            {[
              ...
              <li key={item} className="flex items-start gap-2 text-sm text-slate-700">
                <div className={`w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0 ${i === 0 ? "bg-emerald-500" : "bg-amber-400"}`} />
                {item}
              </li>
            ))}
          </ul>
              ...
              {photos.map((p) => (
                <div key={p.id} className="space-y-1">
                  <div className="relative aspect-square rounded-xl overflow-hidden bg-slate-100">
                    ...
                    <span className="absolute bottom-1 left-1.5 text-white text-[10px] font-semibold">{p.label}</span>
                  </div>
                  <a
                    href={p.dataUrl}
        ...
        <p className="text-xs text-slate-400 text-center">Available online for 90 days (until Jun 10). Download a PDF for your records.</p>
        ...
        <div className="space-y-2.5">
          <button className="w-full bg-navy-900 hover:bg-navy-800 active:scale-[0.98] text-white font-semibold text-sm py-3.5 rounded-xl cursor-pointer transition-all flex items-center justify-center gap-2">
            <Download className="w-4 h-4" />Download Move Summary (PDF)
          </button>
          <button onClick={handleCopy} className="w-full bg-white hover:bg-slate-50 active:scale-[0.98] text-slate-700 font-semibold text-sm py-3.5 rounded-xl border border-slate-200 cursor-pointer transition-all flex items-center justify-center gap-2">
            <Copy className="w-4 h-4" />Copy Share Link
          </button>
          {!showEmailField ? (
            <button onClick={() => setShowEmailField(true)} className="w-full bg-white hover:bg-slate-50 active:scale-[0.98] text-slate-700 font-semibold text-sm py-3.5 rounded-xl border border-slate-200 cursor-pointer transition-all flex items-center justify-center gap-2">
              <Mail className="w-4 h-4" />Email a Copy
            </button>
          ) : (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
              <label htmlFor="email-copy" className="sr-only">Email address</label>
              <input
                id="email-copy" type="email" value={emailInput} onChange={(e) => setEmailInput(e.target.value)}
              ...
              />
              <button
                onClick={handleEmailSend} disabled={!emailInput.trim() || emailSent}
              ...
              >
                <Mail className="w-4 h-4" />{emailSent ? "Sent!" : "Send"}
              </button>
            </motion.div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <h2 className="text-base font-heading font-bold text-navy-900 mb-1">How did we do?</h2>
          <p className="text-slate-600 text-sm leading-relaxed">
            If Marcus and the crew took great care of your move today, a quick Google review helps a small local business more than you know.
          </p>
        ...
        {move.moveHistory.length > 0 && (
          <div>
            <h2 className="text-sm font-heading font-bold text-navy-900 mb-3">Move History for {move.clientName}</h2>
            <div className="space-y-3">
              {move.moveHistory.map((entry) => (
                <HistoryCard key={entry.id} entry={entry} onView={() => router.push("/summary")} />
              ))}
            </div>
            <a href="/" className="mt-4 w-full bg-navy-900 hover:bg-navy-800 active:scale-[0.98] text-white font-semibold text-sm py-3.5 rounded-xl cursor-pointer transition-all flex items-center justify-center gap-2">
              <Truck className="w-4 h-4" />Book Another Move<ArrowRight className="w-4 h-4" />
            </a>
          </div>
        )}
        ...
        <p className="text-center text-xs text-slate-400 mt-2">
          ...
          <a href="https://tododeia.com" className="hover:text-slate-600 transition-colors underline underline-offset-2">Tododeia</a>
        </p>
      </div>
