            ...
            <div className={`pb-5 flex items-start ${i === PIPELINE_STEPS.length - 1 ? "pb-0" : ""}`}>
              <span
                className={`text-sm font-semibold leading-6 ${
                  complete ? "text-navy-800" : current ? "text-amber-600" : "text-slate-400"
                }`}
              >
                {step.label}
                {current && (
          ...
          <>
            ...
            >
              ...
              <div className="flex gap-3 mt-5">
                ...
                <button
                  onClick={() => { approveEstimate(); setShowModal(false); }}
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-100">
        <p className="text-sm font-semibold text-navy-900">Things to know before your move</p>
        <p className="text-xs text-slate-500 mt-0.5">Tailored for: {moveSize} · Local move</p>
      </div>
      <div className="px-4 py-3 space-y-4">
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">What to do first</p>
        ...
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Moving day readiness</p>
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setOpen(false)} className="text-slate-400 cursor-pointer"><X className="w-5 h-5" /></button>
              </div>
              <p className="text-xs text-slate-500 mb-4">
                Current status: <span className="font-semibold text-amber-600">{PIPELINE_STEPS[move.status]?.label}</span>
              </p>
              <div className="grid grid-cols-2 gap-2">
                {(PIPELINE_STEPS.map((s, i) => i) as MoveStatus[]).map((i) => (
                  <button
                    key={i}
                    onClick={() => { setStatus(i as MoveStatus); setOpen(false); }}
                    className={`py-2.5 px-3 rounded-xl text-sm font-medium text-left cursor-pointer transition-all ${
                      move.status === i ? "bg-navy-900 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"
                    }`}
                  >
              ...
              <button
                onClick={() => { advanceStatus(); setOpen(false); }}
...
function CrewPhotos() {
  ...
  const groups = [
    { label: "Before", items: photos.filter((p) => p.label === "Before") },
    { label: "After", items: photos.filter((p) => p.label === "After") },
    { label: "Notes", items: photos.filter((p) => p.label === "Note") },
    ...
    <>
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            <button className="absolute top-4 right-4 text-white/70 hover:text-white cursor-pointer" aria-label="Close">
              <X className="w-6 h-6" />
            </button>
            <motion.img
...
export default function StatusPage() {
  ...
  function handleStartMove() { setStatus(3); }
    ...
    <main className="min-h-screen bg-slate-50 pb-24">
      <header className="bg-navy-900 px-5 pt-4 pb-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <a href="/" className="text-navy-300 text-xs hover:text-white transition-colors">← Edit booking</a>
        </div>
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {isMovingDay && (
                <span className="bg-emerald-500 text-white text-xs font-medium px-2 py-0.5 rounded-full">Moving Day</span>
              )}
            </div>
      ...
      <div className="px-4 py-5 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-navy-900 font-semibold text-sm mt-1">{STATUS_MESSAGES[move.status]}</p>
            </div>
          ...
          {queueLine && (
            <div className="mt-3 pt-3 border-t border-slate-100">
              ...
              <p className="text-slate-400 text-xs mt-0.5">We'll text you when you're next.</p>
            </div>
          )}
        </div>
        ...
        {move.status < 3 && (
          <>
            ...
            {move.status === 2 && (
              ...
              >
                <Truck className="w-5 h-5" />Start Move
              </motion.button>
            )}
            {move.status === 2 && (
              <p className="text-center text-slate-400 text-xs -mt-2">Activates live move-day updates</p>
            )}
          </>
        )}
        ...
        <AnimatePresence>
          {move.status >= 3 && (
            <motion.div key="crew" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.25, 1, 0.5, 1] }}>
              <CrewReveal />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {move.status >= 3 && (
            <motion.div key="estimate" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1, ease: [0.25, 1, 0.5, 1] }}>
              <EstimateCard />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {move.status === 5 && (
            <motion.div key="complete" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: [0.25, 1, 0.5, 1] }}
            ...
            >
              ...
              <p className="text-emerald-700 text-sm mt-1">Thank you for choosing One Stop Moving &amp; Storage.</p>
              <button
              ...
              >
                View Move Summary <ArrowRight className="w-4 h-4" />
              </button>
            </motion.div>
          )}
        </AnimatePresence>
